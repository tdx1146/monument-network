...
class EventConsumer:
    ...
    def __init__(self, event_file: str = _DEFAULT_EVENT_FILE,
        ...
        # 用于获取 event_bus.jsonl 的文件锁（防止并发替换导致数据丢失）
    ...
    def _filter_non_json_lines(self) -> int:
        """
        清除 event_bus.jsonl 中的非 JSON 行（线程安全 + 安全写入）
        使用 LogWriter 的锁防止并发写入导致数据丢失。
        ...
        with self._event_writer._lock:
            ...
            if cleared > 0:
                ...
                # 安全写入：先写临时文件，再原子替换
                tmp_file = self._event_file + ".tmp"
                with open(tmp_file, "w", encoding="utf-8") as f:
                    f.writelines(lines)
                    f.flush()
                    os.fsync(f.fileno())
                os.replace(tmp_file, self._event_file)
                # 问题9修复：过滤后重置 seek 偏移，防止首批事件被跳过
    ...
    def _dispatch(self, event: dict, rule: dict) -> bool:
        ...
        # 旧格式兼容：command（已废弃，打印警告）
        command_template = action.get("command", "")
        if command_template:
            print(f"[event_consumer] ⚠️ 规则 {rule.get('id')} 使用已废弃的 command 格式，"
                  f"请迁移到 exec+script 格式")
            return False

        print(f"[event_consumer] ⚠️ 规则 {rule.get('id')} 无 exec/script 配置")
        return False

    def _dispatch_safe(self, event: dict, rule: dict,
                       exec_cmd: str, script: str) -> bool:
        """安全分派：通过环境变量传递事件数据，无 shell 注入风险。"""
        ...
        # 选择 Python 可执行文件（Windows 兼容）
        python_bin = exec_cmd
        if python_bin == "python3" and os.name == "nt":
