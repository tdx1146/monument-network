...
class RelayServer:
    ...
    def start(self):
        ...
        class RelayHandler(BaseHTTPRequestHandler):
            ...
            def do_OPTIONS(self):
                """处理 CORS 预检请求 — 遵守 RELAY_CORS_ORIGINS 配置"""
                from config import RELAY_CORS_ORIGINS
                origin = self.headers.get("Origin", "")
                if RELAY_CORS_ORIGINS and origin in RELAY_CORS_ORIGINS:
                    self.send_response(200)
                    self.send_header("Access-Control-Allow-Origin", origin)
                    self.send_header("Access-Control-Allow-Methods", "GET, POST, OPTIONS")
                    self.send_header("Access-Control-Allow-Headers", "Content-Type, X-Relay-Key")
                elif not RELAY_CORS_ORIGINS:
                    # 未配置 CORS：允许所有（开发模式）
                    self.send_response(200)
                    self.send_header("Access-Control-Allow-Origin", "*")
                    self.send_header("Access-Control-Allow-Methods", "GET, POST, OPTIONS")
            ...
            def do_GET(self):
                ...
                # API Key 认证（/health 免认证）
                from config import RELAY_API_KEY
                if RELAY_API_KEY and path != "/health":
                    import hmac as _hmac_get
                    key = self.headers.get("X-Relay-Key", "")
                    if not _hmac_get.compare_digest(key, RELAY_API_KEY):
            ...
            def do_POST(self):
                ...
                if RELAY_API_KEY:
                    key = self.headers.get("X-Relay-Key", "")
                    import hmac as _hmac_rel; if not _hmac_rel.compare_digest(key, RELAY_API_KEY):
                        self._send_json({"error": "unauthorized: invalid relay key"}, 401)
                        return
...
def relay_main():
    ...
    if args.daemon:
        if sys.platform == "win32":
            print("[RelayServer] 警告: --daemon 模式在 Windows 上不支持，请使用 pythonw.exe 或直接启动")
        else:
            pid = os.fork()
            if pid > 0:
