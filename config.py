...
# 外部调用（MCP/API）的默认输入置信度
# 注意：这是外部传入的"建议置信度"，不是候选丰碑的决策阈值
# 候选丰碑的决策阈值是 monument_score >= XUANJIAN_MIN_CONFIDENCE（三轴综合得分）
# 两者语义不同：input_confidence 触发评估，monument_score 决定是否创建候选
XUANJIAN_EXTERNAL_DEFAULT_CONFIDENCE = 0.85
...
# ─── 日志轮转配置 ───────────────────────────────────────
LOG_MAX_SIZE_MB = 50             # 单个日志文件最大大小（MB），超过后自动轮转
LOG_RETENTION_COUNT = 5          # 保留的轮转文件数量
EVENT_BUS_MAX_SIZE_MB = 100      # event_bus.jsonl 最大大小（MB），超过后归档
