...
def _start_dht_background(http_address: str, peer_id: str):
    ...
    async def _run_dht():
        ...
        node = DHTNode(storage_dir=str(DHT_STORAGE_DIR))
        global _dht_node_instance
        ...
        await node.start(
            port=DHT_PORT,
            interface="0.0.0.0",
            bootstrap=DHT_BOOTSTRAP_NODES or None,
        )
        logger.info("DHT 节点已启动: DHT-ID=%s, port=%d", node.node_id_hex[:16], DHT_PORT)
        ...
        if discovered:
            logger.info("初始节点发现: 找到 %d 个节点", len(discovered))
