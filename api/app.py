...
def _setup_auth_middleware(app):
    ...
    - 未配置 API_KEY 时为开发模式（仅允许 localhost），打印安全警告
    - /health, /health/simple, /info 免认证
    - 信任的 peer_id 需通过签名验证（不再信任裸 header）
    - 其他请求需要 X-Monument-Key 头匹配 API_KEY（使用恒定时间比较）
    """
    ...
    if not API_KEY:
        warnings.warn(
            "MONUMENT_API_KEY not set! Running in dev mode (localhost only). "
            "Set MONUMENT_API_KEY environment variable for production.",
            stacklevel=2,
        )
    ...
    def _check_api_auth():
        ...
        if not API_KEY:
            ...
            if remote not in ("127.0.0.1", "::1", "localhost"):
                logger.warning("Dev mode: rejected non-localhost request from %s", remote)
                return jsonify({
                    "status": "error",
                    "message": "server in dev mode, localhost only"
                }), 403
            return None

        # 检查 API Key（恒定时间比较，防止时序攻击）
        key = request.headers.get("X-Monument-Key", "")
        if _hmac.compare_digest(key, API_KEY):
            return None

        # 信任的 peer_id：需要同时提供有效签名（X-Monument-Sig 头）
        # 裸 X-Monument-Peer 头不再可信，防止伪造
        peer_id = request.headers.get("X-Monument-Peer", "")
        peer_sig = request.headers.get("X-Monument-Sig", "")
        if peer_id and peer_id in API_TRUSTED_PEERS and peer_sig:
            # 签名验证：peer_sig = HMAC(peer_id, API_KEY)
            expected_sig = _hmac.new(
                API_KEY.encode("utf-8"),
                peer_id.encode("utf-8"),
                "sha256"
            ).hexdigest()
            if _hmac.compare_digest(peer_sig, expected_sig):
                return None

        # 认证失败
        logger.warning("API auth failed: path=%s remote=%s", path, request.remote_addr)
        return jsonify({
            "status": "error",
...
def create_app() -> Flask:
    ...
    # 从 config 读取轮转参数（问题11修复）
    try:
        from config import LOG_MAX_SIZE_MB, LOG_RETENTION_COUNT
    except ImportError:
        LOG_MAX_SIZE_MB = 50
    ...
    # 问题10修复：同时挂载到 monument-api logger，防止 INFO 日志丢失
