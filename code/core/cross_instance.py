...
def import_monument_json(
    ...
    if score_data:
        ...
        score_book = LocalScoreBook(score_repo=score_repo)
        
        # 使用归一化后的健康分
        health_score = score_data.get("health_score", 0)
        if health_score > 0:
            score_book.add_generic_score(
                ai_id=ai_id,
                delta=health_score,
