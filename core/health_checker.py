...
class HealthChecker:
    ...
    def check_disk(self) -> CheckResult:
        ...
        try:
            # 使用 os.statvfs（POSIX）获取磁盘信息
            st = os.statvfs(self.data_dir)
            free_bytes = st.f_frsize * st.f_bavail     # 实际可用给非 root 的字节
            total_bytes = st.f_frsize * st.f_blocks     # 总字节
            free_mb = free_bytes / (1024 * 1024)
            total_mb = total_bytes / (1024 * 1024)
            used_mb = total_mb - (st.f_frsize * st.f_bfree / (1024 * 1024))
        ...
        try:
            with open("/proc/net/tcp", "r") as f:
                for line in f.readlines()[1:]:  # 跳过表头
                    ...
                    # 0A = TCP_LISTEN
                    if state == 0x0A and local_addr.split(":")[1] == hex_port:
                        return True
